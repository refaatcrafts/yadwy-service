package yadwy.app.yadwyservice.product.infrastructure.controllers

import app.yadwy.api.UpdateProductApi
import app.yadwy.model.ProductResponseDto
import app.yadwy.model.UpdateProductRequestDto
import org.springframework.http.ResponseEntity
import org.springframework.security.core.context.SecurityContextHolder
import org.springframework.security.oauth2.jwt.Jwt
import org.springframework.web.bind.annotation.RestController
import yadwy.app.yadwyservice.product.application.models.UpdateProductRequest
import yadwy.app.yadwyservice.product.application.usecases.UpdateProduct
import yadwy.app.yadwyservice.product.domain.contracts.SellerGateway
import yadwy.app.yadwyservice.product.domain.exceptions.SellerNotFoundException
import java.math.BigDecimal

@RestController
class UpdateProductController(
    private val updateProduct: UpdateProduct,
    private val sellerGateway: SellerGateway
) : UpdateProductApi {

    override fun updateProduct(
        id: Long,
        updateProductRequestDto: UpdateProductRequestDto
    ): ResponseEntity<ProductResponseDto> {
        val jwt = SecurityContextHolder.getContext().authentication?.principal as Jwt
        val accountId = jwt.getClaim<Long>("id")
        val seller = sellerGateway.findSellerIdByAccountId(accountId)
            ?: throw SellerNotFoundException(accountId)

        val request = UpdateProductRequest(
            productId = id,
            sellerId = seller.id,
            name = updateProductRequestDto.name?.toDomain(),
            description = updateProductRequestDto.description?.toDomain(),
            images = updateProductRequestDto.images,
            price = updateProductRequestDto.price?.let { BigDecimal.valueOf(it) },
            compareAtPrice = updateProductRequestDto.compareAtPrice?.let { BigDecimal.valueOf(it) },
            stock = updateProductRequestDto.stock,
            trackInventory = updateProductRequestDto.trackInventory,
            visible = updateProductRequestDto.visible
        )

        val response = updateProduct.execute(request)

        return ResponseEntity.ok(response.toDto())
    }
}
