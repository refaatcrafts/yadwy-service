...
@Component
class CustomerAccountCreatedPublisher(
    private val applicationEventPublisher: ApplicationEventPublisher,
    private val objectMapper: ObjectMapper
) {
    private val logger = LoggerFactory.getLogger(CustomerAccountCreatedPublisher::class.java)

    fun publish(event: AccountCreatedEvent) {
        logger.info("Publishing CustomerAccountCreated for accountId: {}", event.accountId.id)

        val dto = CustomerAccountCreatedDto(
